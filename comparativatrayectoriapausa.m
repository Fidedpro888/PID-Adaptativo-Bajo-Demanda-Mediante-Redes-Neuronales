% COMPARACION 3 CONTROLADORES EN YOUBOT (TRAYECTORIA ROSA + PAUSA DURA)
% PID convencional vs EKF-SNPID (básico) vs Propuesto (EKF-SNPID + freeze)
% - Misma referencia "rosa", con PAUSA dura (mantener ref) y BLOQUEO dinámico (mantener estado)
% - Saturación común (+/- 0.3)
% - Figuras: Seguimiento (3 subplots), Errores (subplots), Control (subplots)
...
r   = 0.0475;      % m
dt  = 0.05;        % s
Tsim= 130;         % s (duración base ANTES de insertar la pausa)
N0  = round(Tsim/dt);
t0  = (0:N0-1)*dt;

sat_limit = 0.3;   % saturación común para los 3 controladores

%% Referencia ROSA (versión compacta) EN TIEMPO BASE (sin pausa)
a0     = 0.2 + 0.05*cos(3*0.05*t0);
xref_0 = a0 .* cos(0.05*t0) - 0.25;
yref_0 = a0 .* sin(0.05*t0);
thref_0= pi/4 * ones(1, N0);

%% Inserción de PAUSA DURA "como estaba originalmente"
pause_start    = 35;   % s  (instante en el que se congela la referencia)
pause_duration = 50;   % s  (duración de la pausa)
idx_start = round(pause_start/dt);      % índice de inicio (estilo original)
idx_pause = round(pause_duration/dt);   % muestras adicionales insertadas
idx_end   = idx_start + idx_pause;

% Valores a mantener durante la pausa
x_hold  = xref_0(idx_start);
y_hold  = yref_0(idx_start);
th_hold = thref_0(idx_start);

% Construir nuevas referencias con segmento mantenido
xref = [xref_0(1:idx_start), repmat(x_hold, 1, idx_pause), xref_0(idx_start+1:end)];
yref = [yref_0(1:idx_start), repmat(y_hold, 1, idx_pause), yref_0(idx_start+1:end)];
thref= [thref_0(1:idx_start), repmat(th_hold,1, idx_pause), thref_0(idx_start+1:end)];

% Nuevo tiempo total (se alarga en pause_duration)
...
for k = 2:N
    ...
    % BLOQUEO dinámico durante la pausa
    if k >= idx_start && k < idx_end
        state_PID(:,k) = state_PID(:,k-1); % bloqueado
    else
...
% Pesos iniciales solicitados (independientes)
w_xp = [0.3; 0.2; 0.2];
w_yp = [0.3; 0.2; 0.1];
w_tp = [0.2; 0.1; 0.05];
...
alpha_prop = 1.5;     
e_thr_freeze = 0.03;   % congela si |e| < 0.05
e_thr_unfz   = 0.05;   % descongela si |e| > 0.15
...
for k = 2:N
    ...
    % Integración de I con gating por movimiento (como en tu propuesto)
    if k>=3
...
disp('==== METRICAS COMPARATIVAS (Trayectoria Rosa + Pausa) ====');
disp(metricsTbl);
...
function M = compute_metrics(t, ex, ey, eth, ux, uy, uth, dt)
    ...
    % % dentro de tolerancias: 5 cm y 5°
    tol_pos = 0.05;            % m
    tol_th  = 5*pi/180;        % rad
